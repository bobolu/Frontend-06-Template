...
function emit(token) {
  ...
  if (token.type === "startTag") {
    ...
    // element.parent = top; // 这句不注释，JSON.stringify(dom)输出会报错。
    ...
    if (top.tagName === token.tagName) {
      ...
      // 可提高容错性，比如结束节点的时候，自动闭合内部未闭合的标签等
    }
    currentTextNode = null;
...
function match(ele, selector) {
  ...
  // 还能扩展支持更多选择器，此处实现三种简单选择器
  return false;
}

function computeCSS(ele) {
  // console.log(rules)
  // console.log("compute CSS for Element", ele);
  ...
  for(let rule of rules) {
    ...
    if(matched) {
      ...
      for(let declar of rule.declarations) {
        ...
        } else if(compare(computedStyle[declar.property].specificity, sp) < 0) {
          computedStyle[declar.property].value = declar.value;
          computedStyle[declar.property].specificity = sp;
        }
      }
      // console.log(ele.computedStyle);
    }
  }
}
